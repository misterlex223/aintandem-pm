/**
 * CoworkingSpace E2E Test
 *
 * End-to-end test for CoworkingSpace functionality with actual providers.
 *
 * Usage:
 *   cd repos/ce-orchestrator && npx ts-node project/tests/e2e/coworking-space/test-coworking-space.ts
 *
 * Prerequisites:
 *   - Build the project: pnpm build
 *   - Local Docker or Aliyun ECS available
 */
...
import { SpaceNetworkingService } from '../../../../repos/ce-orchestrator/src/services/space-networking';
...
async function testCreateLocalSpace() {
  ...
  // Get space status
  info('Getting space status...');
  try {
    const status = await manager.getSpaceStatus(space.id);
    success(`Space status: ${status.status}`);
    success(`Providers: ${status.providers.length}`);
    success(`Containers: ${status.providers.reduce((sum, p) => sum + p.containerCount, 0)}`);
    passedTests++;
  } catch (err) {
...
// Main test runner
async function main() {
  section('CoworkingSpace E2E Test');
  log('Testing CoworkingSpace functionality with actual providers', colors.yellow);
  console.log('');
